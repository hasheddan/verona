  ...
  public:
    ...
    backpressure_scan(const MessageBody& senders, const MessageBody& receivers)
    {
...
#endif
      // TODO: why is this needed? Why isn't load_reset used enough with
      // multiple participants in message?
      const uint32_t load_compensation =
        (receivers.count == 1) ? 0 : (0xf0 * 4);
      ...
      for (size_t r = 0; r < receivers.count; r++)
      {
        ...
        auto load = rt::bits::backpressure_load(bp);
        load += load_compensation;
        pressure += load >> 3;
      ...
      // TODO: faster approximate average?
      pressure /= receivers.count;
      ...
      for (size_t s = 0; s < senders.count; s++)
      {
        ...
        logger::cout().trace(
          "backpressure_pressure", sender, sender_pressure, now);
#endif
      }
    }
    ...
    bool run(Alloc* alloc, ThreadState::State, EpochMark)
    {
      ...
      do
      {
        ...
        if ((bp & 0xff) == 0)
        {
          ...
          // TODO: avoid allocating this every time
        ...
        // TODO: adjust batch size based on load?
      } while ((curr != until) && (batch_size < 128));
